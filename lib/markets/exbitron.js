...
function get_summary(coin, exchange, cb) {
  var summary = {};
  request({uri: base_url + '/'+ "xrgbch" + '/tickers', json: true}, function (error, response, body) {
    console.log(body);
    if (error) {
      ...
      if (orders['bids'].length > 0){
        for (var i = 0; i < orders['bids'].length; i++) {
          var order = {
            ...
            total: (parseFloat(orders.bids[i]["remaining_volume"]).toFixed(8) * parseFloat(orders.bids[i]["remaining_volume"])).toFixed(8)
          }
          buys.push(order);
        }
      } else {}
